...
использует только API/application каталоги страницы «Поиск», ищет в их уже
существующем cache текущего environment по `ReferenceConfig.search_keys`, не
обновляет даже просроченный cache и ничего не загружает по сети. Справочники
полей форм и плагинов в picker не попадают. Backend повторно разрешает pointer и
передаёт Copilot
