...
def show_loading(
    ...
    """
    Показывает немодальное окно загрузки с анимацией braille-спиннера.

    Возвращает функцию close(), безопасную для вызова из фонового потока.

    Без worker — ручное управление:
    ...
    С worker — поток запускается автоматически:
        show_loading(
            self, "Загрузка данных...",
            worker=lambda: fetch_data(),
            on_done=lambda result: self._render(result),
    ...
    content = tk.Frame(dlg, bg=theme.C["bg"])
    content.pack(padx=32, pady=24)

    spinner_lbl = tk.Label(
        content, text=_FRAMES[0],
        font=("Consolas", 20), bg=theme.C["bg"], fg=theme.C["primary"],
    )
    spinner_lbl.pack()
    ...
    def _animate() -> None:
        if _closed[0] or not dlg.winfo_exists():
            return
        _idx[0] = (_idx[0] + 1) % len(_FRAMES)
        spinner_lbl.config(text=_FRAMES[_idx[0]])
        dlg.after(80, _animate)
...
# -------------------------------------------------------------------------
# Информационное окно после завершения опроса
# -------------------------------------------------------------------------
...
def show_polling_info(
    ...
    """
    Показывает независимое (немодальное) окно с информацией после завершения опроса.

    Отличие от show_text_viewer: не блокирует родительское окно (нет grab_set/wait_window),
    т.е. пользователь может взаимодействовать с окном результата одновременно.
    Содержит кнопку «Скопировать» для копирования всего текста в буфер обмена.

    Пример использования из ResultScreen — задаётся в форме через show_info_after_polling.
    """
    dlg = tk.Toplevel(parent)
    dlg.title(title)
    dlg.configure(bg=theme.C["bg"])
    dlg.minsize(540, 360)
    dlg.transient(parent.winfo_toplevel())
    # Без grab_set() — окно независимое, не блокирует родительский экран
    ...
    theme.separator(dlg, pady=4)

    btn_row = tk.Frame(dlg, bg=theme.C["bg"])
    btn_row.pack(pady=(0, 12))

    copy_var = tk.StringVar(value="Скопировать")

    def _copy() -> None:
        dlg.clipboard_clear()
        dlg.clipboard_append(text)
        copy_var.set("Скопировано ✓")
        dlg.after(1500, lambda: copy_var.set("Скопировать"))

    ttk.Button(btn_row, textvariable=copy_var, style="Secondary.TButton",
               command=_copy).pack(side=tk.LEFT, padx=(0, 8))
    ttk.Button(btn_row, text="Закрыть", style="Ghost.TButton",
               command=dlg.destroy).pack(side=tk.LEFT)
