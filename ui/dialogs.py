...
def show_refresh_confirm(
    ...
    Показывает когда последний раз обновлялся кеш и предлагает
    обновить или отменить.

    cached_at — unix-timestamp последнего обновления (None если кеша нет).
    Возвращает True если пользователь нажал «Обновить».

    Пример:
        ts = app.reference_cache.get_timestamp(resource, env)
        if show_refresh_confirm(self, field_def.label, ts):
            ...
    """
    result: list[bool] = [False]
    ...
    # Блок с датой последнего обновления
    ...
    if cached_at is not None:
        dt = datetime.fromtimestamp(cached_at)
