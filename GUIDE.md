...
- [4. Добавить новую форму / изменить существующую](#4-добавить-новую-форму--изменить-существующую)
...
- [6. Экран результата после сабмита](#6-экран-результата-после-сабмита)
...
- [7. UI-утилиты: диалоги](#7-ui-утилиты-диалоги)
...
- [8. Переключение справочников: HTTP ↔ локальный](#8-переключение-справочников-http--локальный)
...
- [9. Авторизация HTTP-справочников](#9-авторизация-http-справочников)
...
- [10. Кеширование HTTP-справочников](#10-кеширование-http-справочников)
...
- [11. ITSM-интеграция: подтянуть данные из заявки](#11-itsm-интеграция-подтянуть-данные-из-заявки)
...
- [12. Сервисы: GraviteeService, TfsService, ITSMService](#12-сервисы-graviteeservice-tfsservice-itsmservice)
- [13. Предыдущие запуски](#13-предыдущие-запуски)
...
`OperationsScreen` — карточный экран, открывающийся при нажатии «📋 Операции» на главном экране. Каждая операция отображается карточкой; нажатие на карточку переходит на соответствующий экран.

```
OperationsScreen
 ├── ⚡ Операция с АПИ → ApiOpScreen
 └── ... (ваши операции)
```

Список операций задаётся константой `_OPERATIONS` в `ui/screens/operations_screen.py`:

```python
_OPERATIONS: List[Tuple[str, str, str, str]] = [
    # (иконка, заголовок, описание, ключ)
    ("⚡", "Операция с АПИ", "Выбрать АПИ и выполнить действие", "api_op"),
]
```

Когда список пуст — показывается заглушка «Раздел в разработке». При наполнении автоматически появляются карточки.

---

### Добавить новую операцию

**Шаг 1.** Добавить карточку в `_OPERATIONS` в `ui/screens/operations_screen.py`:

```python
_OPERATIONS: List[Tuple[str, str, str, str]] = [
    ("⚡", "Операция с АПИ", "Выбрать АПИ и выполнить действие", "api_op"),
    ("📤", "Моя операция",   "Краткое описание",                  "my_op"),  # ← добавить
]
```

Кортеж: `(иконка, заголовок, описание, ключ)`. Ключ должен быть уникальным.

**Шаг 2.** Добавить ветку в `_open()` в том же файле:
...
def _open(self, key: str) -> None:
    if key == "api_op":
        ...
        from ui.screens.operations.my_op_screen import MyOpScreen
        self.app.navigate_to(MyOpScreen)
```

**Шаг 3.** Создать файл `ui/screens/operations/my_op_screen.py` — наследник `BaseScreen`:
...
### Использовать справочник в операции

Для выбора элемента из справочника (АПИ, приложение и т.п.) — использовать диалоги `ask_dictionary` / `ask_multi_dictionary` из `ui/dialogs.py`:
...
class MyOpScreen(BaseScreen):
    ...
    def _pick_api(self) -> None:
        # Одиночный выбор → Optional[str] (value_key выбранного элемента)
        api_id = ask_dictionary(
            self, "Выбрать АПИ",
            reference=_API_REF,
            environment=self.app.current_environment.get(),
        ...
        if api_id is not None:
            ...  # пользователь выбрал api_id

    def _pick_apis(self) -> None:
        # Мультивыбор → Optional[List[str]]
        api_ids = ask_multi_dictionary(
            self, "Выбрать АПИ",
            reference=_API_REF,
            environment=self.app.current_environment.get(),
            app=self.app,
        ...
        if api_ids is not None:
            ...  # список выбранных id, может быть пустым
```

`ask_dictionary` / `ask_multi_dictionary`:
- Загружают данные через `app.reference_resolver` — с кешированием по тем же правилам, что и справочники внутри форм (см. раздел 10)
- Прокручиваемый список с поиском по `search_keys`
...
Ресурс `infra_resources` нужно добавить в `_URL_MAP` в `handlers/http_reference_handler.py` (см. раздел 8).
...
## 4. Добавить новую форму / изменить существующую
...
## 5. Изменить обработку сабмита и конструирование JSON
...
## 6. Экран результата после сабмита
...
## 8. Переключение справочников: HTTP ↔ локальный
...
## 9. Авторизация HTTP-справочников
...
## 10. Кеширование HTTP-справочников
...
## 11. ITSM-интеграция: подтянуть данные из заявки
...
## 12. Сервисы: GraviteeService, TfsService, ITSMService
...
## 14. Настройки: обязательные поля

При каждом запуске приложения проверяется заполнение обязательных полей `.env`.
Если поле не заполнено — автоматически открывается `SettingsScreen` с баннером-предупреждением и фокусом на нужном поле ввода.

### Как объявить обязательное поле

В [config/environments.py](config/environments.py) добавьте кортеж в `REQUIRED_SETTINGS`:

```python
# config/environments.py

REQUIRED_SETTINGS: List[Tuple[str, str]] = [
    (LOGIN_KEY,     "Логин"),              # ← уже есть по умолчанию
    (TFS_TOKEN_KEY, "TFS Token"),          # ← раскомментируйте / добавьте нужные
    (CERT_PATH_KEY, "Путь к сертификату"),
]
```

Формат каждого элемента: `(ключ_из_env, человекочитаемое_название)`.
Ключи-константы определены там же в `environments.py`: `LOGIN_KEY`, `TFS_TOKEN_KEY`, `CERT_PATH_KEY`, `GRAVITEE_REPO_PATH_KEY`, `ITSM_LOGIN_KEY`, `ITSM_PASSWORD_KEY`, а также `gravitee_token_key(env.key)` для токенов по окружениям.

### Как работает проверка

1. После запуска `HomeScreen` вызывается `app._check_required_settings()` через `root.after(0, ...)`.
2. Метод перебирает `REQUIRED_SETTINGS` **по порядку** и проверяет `value.strip()`.
3. При первом незаполненном поле выполняется `navigate_to(SettingsScreen, missing_key=..., missing_label=...)`.
4. `SettingsScreen` показывает баннер с названием поля и фокусируется на нужном `Entry` (с автоскроллом).
5. После сохранения пользователь возвращается на `HomeScreen` обычным способом (`go_back()`).

> Проверка выполняется **один раз** при запуске. Если пользователь пропустил настройку и вернулся на главный экран — повторной проверки не будет до следующего запуска приложения.
...
При активном соединении на главной появляется единый чат `Gravitee AI`. Агент
`autodeploy-copilot` по закрытой JSON Schema выбирает форму, строит зависимый
многоформовый план, ищет API/приложения обычным языком, находит похожие шаблоны и
разбирает ошибки исполнения. Консервативный порог разрешает автоматический выбор
формы только для однозначного результата; иначе пользователь выбирает одного из
трёх кандидатов. Затем запускается существующий `FormExtractorAgent` и тот же
inline review flow, без повторного получения ITSM/ADO-контекста.
...
подтверждения пользователя. `diagnose_search` и неизвестные инструменты остаются
запрещены.
...
Малые локальные и HTTP-справочники (по умолчанию меньше 100 элементов, не более
24 KiB каждый и 48 KiB суммарно) передаются form-extractor как безопасная
проекция `value/label/aliases`; их ID одновременно становятся enum JSON Schema.
Лимиты меняются на экране **OpenCode**. Крупные и пока неразрешимые зависимые
справочники в prompt не отправляются: агент сохраняет смысловые названия, а
Python локально сопоставляет их с ID и затем выполняет строгую schema- и
domain-валидацию. Предложения временно появляются непосредственно в полях формы;
отправка блокируется, пока оператор не примет или не отклонит их по отдельности
либо общим действием. Уточнение выполняется в той же extractor-session.
...
├── .opencode/
│   └── agents/
│       ├── autodeploy-copilot.md  # primary agent: единый чат, поиск, планы, диагностика
...
│   ├── mcp_profiles.py             # точные allow/ask-профили JSON Repository MCP
...
└── ui/
    ...
    ├── inline_ai_review.py        # обратимое состояние inline-проверки AI-полей
