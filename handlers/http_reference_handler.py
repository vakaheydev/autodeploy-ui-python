...
class HttpReferenceHandler(BaseReferenceHandler):
    ...
    def _filter_items(
        ...
        result = fn(environment, items)

        # Защита от случайного [[item, ...]] вместо [item, ...]
        # — возникает если лямбда оборачивает list comprehension в лишние []
        if result and isinstance(result[0], list):
            _log.error(
                "_FILTER_MAP['%s'] вернул список списков ([[...]] вместо [...]).\n"
                "Проверьте лямбду: нужно [item for item in items if ...],\n"
                "а не [[item for item in items if ...]].\n"
                "Применяю unwrap автоматически.",
                resource,
            )
            result = result[0]
